...
export function AttendanceDivisionFilter() {
  const { data: divisions } = useDivisions();
  const { get, set, remove } = useQueryParams();
  const selectedDivision = get("division");

  return (
    <ScrollArea className="pb-4 whitespace-nowrap w-full mt-4">
      <div className="flex w-max space-x-2">
        ...
        {divisions
          .filter((division) => division.id !== GENERAL_DATA_ID)
          .map((division) => {
            ...
            return (
              <Badge
                variant="outline"
                key={division.id}
                onClick={() => set("division", division.name)}
                className={cn(
                  "cursor-pointer text-white",
