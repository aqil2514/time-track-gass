...
export const getColumns = (): ColumnDef<AuthUser>[] => [
  {
  ...
  },
  {
    ...
    header: "Email",
    cell: ({ row }) => (
      <span className="text-slate-300">{row.getValue("email")}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "Role",
    cell: ({ row }) => (
      <span className="capitalize text-slate-400">{row.getValue("role")}</span>
    ),
  },
  {
    id: "actions",
    ...
    cell: ({ row }) => (
      <div className="text-right">
        <TeamsTableActions user={row.original} />
      </div>
    ),
  },
];
