...
import { ZAIHelperService } from './general.service';
...
export class AnalyzeImageService {
  ...
  async getAiImageAnalyze(
    imageDataUrl: string,
    userId: string,
  ): Promise<ZImageAnalyzeReturn> {
    const prompt = await this.buildPrompt(userId);
    ...
    try {
      const { rawText, cleanJson, reasoning } =
        await this.helper.chatCompletion(content);
