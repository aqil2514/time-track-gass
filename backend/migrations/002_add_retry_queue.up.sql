...
CREATE INDEX idx_retry_queue_status_pending ON retry_queue (status, next_retry_at) WHERE status = 'pending';
CREATE INDEX idx_retry_queue_task_type ON retry_queue (task_type, status);
