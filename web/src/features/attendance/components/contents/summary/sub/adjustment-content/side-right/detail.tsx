...
const Header: React.FC<{ selected: AdjustmentContentDetail }> = ({
  selected,
}) => {
  return (
...
const EmployeeInfo: React.FC<{ selected: AdjustmentContentDetail }> = ({
  selected,
}) => {
  return (
    <div className="grid gap-3 rounded-2xl border border-slate-700/60 bg-slate-950/60 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">{selected.profile.full_name}</p>
        </div>
...
const AffectedMinutes: React.FC<{ selected: AdjustmentContentDetail }> = ({
  selected,
}) => {
  return (
...
export function SideRightDetail() {
  ...
  if (!state.adjustmentId) return <NoAdjustmentID />;
    ...
    <>
      ...
      <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-6 shadow-sm shadow-slate-950/20">
        ...
        <div className="mt-6 grid gap-4">
          <EmployeeInfo selected={selected} />

          <AffectedMinutes selected={selected} />
